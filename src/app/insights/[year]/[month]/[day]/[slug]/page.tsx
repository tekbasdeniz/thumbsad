...
export async function generateStaticParams() {
    try {
        ...
        console.error('Error generating static params for insights (TR):', error);
        return [];
    }
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    try {
        ...
        if (!resolvedParams?.slug || !resolvedParams?.year || !resolvedParams?.month || !resolvedParams?.day) {
            return { title: 'İçgörü Bulunamadı | ThumbsAd' };
        }
        ...
        return {
            title: `${post?.title || 'İçgörüler'} | ThumbsAd İçgörüler`,
            description: post?.excerpt_tr || post?.excerpt || undefined,
        };
    } catch (error) {
        console.error('Error generating metadata for insights detail (TR):', error);
        return { title: 'İçgörüler | ThumbsAd' };
    }
}
...
export default async function Page({ params }: PageProps) {
    ...
    try {
        ...
        console.error('Sanity fetch error on insights detail page (TR):', error);
        post = null;
    }
    ...
    const categoryName = post?.category_tr || post?.category || post?.categories?.[0]?.title || 'İçgörüler';
