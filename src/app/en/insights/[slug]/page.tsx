...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
    ...
    if (!post) {
        return { title: 'Post Not Found | ThumbsAd' };
    }
    return {
        title: `${post.en.title} | ThumbsAd Insights`,
        ...
        <main className="w-full bg-white">
            <article className="w-full max-w-4xl mx-auto px-6 py-24 md:py-32">
                ...
                <div className="space-y-6 mb-10">
                    <Link href="/en/insights" className="text-[#0a246b] font-semibold text-sm hover:underline inline-flex items-center gap-2">
                            ...
                            <ul className="space-y-6 my-8 list-none pl-0">
                                {content.applications.map((app, index) => (
                                    <li key={index} className="flex gap-4 items-start rounded-xl p-6 bg-white border border-gray-100 shadow-sm">
                                        <div className="mt-1 w-8 h-8 rounded-full bg-blue-100 text-[#0a246b] flex items-center justify-center shrink-0">
